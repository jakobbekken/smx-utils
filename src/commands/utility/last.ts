  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const title = data[0].song.title;
        const difficultyDisplay = data[0].chart.difficulty_display;
        const difficulty = data[0].chart.difficulty;
        const smxUsername = data[0].gamer.username;
      
        await interaction.reply(`
          Last song played by ${smxUsername} was ${title} on ${difficultyDisplay} ${difficulty}
        `);
